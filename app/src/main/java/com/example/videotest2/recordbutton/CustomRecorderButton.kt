...
class CustomRecorderButton @JvmOverloads constructor(
    context: Context,
    ...
    companion object {
        ...
        private const val VIDEO_DURATION = 2000L//2s

        private const val RECORD_STATUS_START = "start"
        private const val RECORD_STATUS_PAUSE = "pause"
        private const val RECORD_STATUS_RESUME = "resume"
        private const val RECORD_STATUS_STOP = "stop"
    }
    ...
    private var innerCirclePaint = Paint().apply {
        ...
        innerCircleFillColor = ContextCompat.getColor(getContext(), R.color.white)
    ...
    private var outerCirclePaint = Paint().apply {
        isAntiAlias = true
        style = Paint.Style.FILL
        outerCircleFillColor = ContextCompat.getColor(getContext(), R.color.white)
    ...
    private fun onClickEnd() {
        ...
        if (isRecordTooShort(startRecordTime, endRecordTime)) {
    ...
    private fun isRecordTooShort(startMillis: Long, endMillis: Long) =
        endMillis - startMillis < MINIMUM_VIDEO_DURATION_MILLIS
    ...
    private fun calculateAngle(startTime: Long, stopTime: Long): Float {
        ...
        val calculateAngle = millisPassed * 360f / videoDurationInMillis
        if (millisPassed > 0) {
            actionListener?.getTime(millisPassed)
        }
